    ...
    <WorkspaceLayout sectionTitle="Target Companies" sectionSubtitle="Ground mock interviews in official employer specs">
      ...
      {loading ? (
        ...
        <div className="saas-card" style={{ padding: "3rem", textAlign: "center" }}>
          <Building2 size={36} color="var(--text-tertiary)" style={{ marginBottom: "0.5rem" }} />
          <h3 style={{ fontSize: "1rem", color: "var(--text-primary)" }}>No companies cataloged yet</h3>
          <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>Check back later or configure your target roles directly.</p>
        </div>
      ) : (
              ...
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                    <div style={{ backgroundColor: "var(--bg-subtle)", padding: "0.4rem", borderRadius: "8px", border: "1px solid var(--border-subtle)", color: "var(--text-primary)" }}>
                      <Building2 size={18} />
                    </div>
                    <div>
                      <h3 style={{ fontSize: "1.05rem", fontWeight: 600, color: "var(--text-primary)", margin: 0 }}>{c.name}</h3>
                      {c.website && (
                ...
                <p style={{ color: "var(--text-muted)", fontSize: "0.825rem", marginBottom: "0.85rem", lineHeight: 1.5 }}>
                  {c.description}
                </p>

                {c.culture_keywords && c.culture_keywords.length > 0 && (
                  <div style={{ marginBottom: "1.25rem" }}>
                    <span style={{ fontSize: "0.7rem", color: "var(--text-muted)", textTransform: "uppercase", display: "block", marginBottom: "0.3rem", fontWeight: 600 }}>Culture & Tech Keywords</span>
